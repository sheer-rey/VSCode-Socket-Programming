...
namespace sheer_rey {
  ...
  /*****************************************************************************
   * @name GetInfixExpression
   * @return infix expression from standard input
   * @brief Get infix expression from standard input and return string object
   * **************************************************************************/
  string GetInfixExpression() {
    ...
    while (true) {
      ...
      if (!str.size())
        cerr << "Error, empty line! Reinput the 'infix expression' send to "
                "server ("
             << cMaxBufferSize - 1
             << " characters max, press enter to send, q to quit): ";
   ...
   * **************************************************************************/
  int SendInfixExpression(SOCKET& _client_socket,
                          const string& _infix_expression) {
    /* pack the input line and sent to server */
    PackageCalculator infix(_infix_expression.c_str());

    if (send(_client_socket, (char*)&infix, sizeof(infix), 0) == SOCKET_ERROR) {
   ...
   * show
   * **************************************************************************/
  int ShowCalculationResult(SOCKET& _client_socket) {
    ...
    while (receive_lenth < int(sizeof(package_header))) {
      receive_status =
          recv(_client_socket, (char*)&package_header + receive_lenth,
    ...
    if (package_header.command == CMD_Calculator) {
      ...
      switch (calculator.calculation_status) {
        ...
        case cUnknownError: cerr << "Error! Unknown error." << endl; break;
   ...
   *       cMaxBufferSize), then send to server specified by \p _client_socket.
   *       Finally get the echo message from server and output.
   * **************************************************************************/
  int EchoMessageClient(SOCKET& _client_socket) {
    cout << "Please enter the message send to server (" << cMaxBufferSize - 1
         << " characters max, press enter to send): ";
    ...
    if (str.size() > cMaxBufferSize - 1) {
      cerr << "Warning: the input is greater than 127 characters, it will be "
              "trimmed."
           << endl;
  ...
  int CalculatorClient(SOCKET& _client_socket) {
    if (SendInfixExpression(_client_socket, GetInfixExpression()) == -1)
      return -1;
    if (ShowCalculationResult(_client_socket) == -1) return -1;
    return 0;
  }
}  // namespace sheer_rey
